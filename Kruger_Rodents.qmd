...
specNames <- names( specSum )[ specSum > 5 ]
...
beta.tune <- c(.3,.0005)                         # SD for beta proposals
...
lnames <- as.vector( t( outer( toupper( letters[1:5] ), 1:5, paste, sep = '' ) ) ) # grid names
id <- colnames( yt )                                                               # individuals
n  <- length( id )
L  <- nrow( X )
Y  <- matrix( 0, L, n, dimnames = list( lnames, id ) )               # where each ID caught
...
```{r}
tmp <- fitModel( Y, J, M, X, A, n.mcmc, mu.beta, beta.tune, s2.beta, n.sim, mpath = '' ) # MCMC 
...
yMean <- matrix( 0, length( plotYears ), nspec, 
                 dimnames = list( plotYears, specNames ) )
ySE <- yMean
